            ...
            <div className='flex flex-col justify-around items-center md:flex-row'>  
                {categories.map((category, index) => (  
                    <div onClick={()=>{handleSearch(category.id)}}>
                        <div key={index} className="w-full ml-5 my-5 bg-white rounded-2xl shadow-lg p-8 flex flex-col items-center cursor-pointer transition-transform duration-200 hover:scale-105" onClick={() => <Link href={`/products?categoryId=${category.id}`}></Link>}> 
